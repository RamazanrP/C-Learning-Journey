...
while (elverisli == 0) {
    ...
    for(int i=0; i<N; i++) { //Hist mantığımız
        scanf("%d", &dizi[i]);
        alan[i]=0;
    }
    for(int i=0; i<N; i++) {
        if (dizi[i] <0) {
         ...
         for (int j= i-etki; j<= i+etki; j++) {
            if (j >= 0 && j < N) {
                if(alan [j] ==1) { //İlk alınan bomba buraya girmez, else'e girip kendi çevresini 1 yazdırır, Başka bomba varsa ve çakışma varsa elverisli değil
                    cakisma =1;
                } else {
                    alan[j] =1; 
                }
...
for (int i=0; i<N; i++) {
    if(alan[i]==0) { //hist mantığı ile 0lamıştık en başta, bir olmayanları yazdırıyoruz
        printf("%d ", dizi[i]);
    }
}
return 0;

...
void yazdir_tahta() {
    if( mod == 1){
    	system("cls"); //Sadece Oyun modunda ekranı temizliyoruz. Kontrol modunda tüm değişikler ekranda gösterilsin. Kurallarda öyle var!
	}
	printf("\n   ");
    int i,j;
...
void yer_cekimi_uygula() {
	int hareket_var;
	do { // Taşların düşüp düşmeyeceğini bilemeyiz. O yüzden en az 1 kere baştan sona taramalıyız
		hareket_var = 0;
		int i,j;
		...
		system("cls"); // Üst üste uygun ikili kalmadığında artık nihai tahtayı yazdıralım
		yazdir_tahta();
...
int main() {
   ...
   for( i=0; i<N; i++) for( j=0; j<M; j++) tahta[i][j] = ' ';
    ...
    if (mod == 1) { 
    	...
    	printf("Kontrol modunu sectiniz \n");
    	printf("Gecerli karakterler: *,/,+,O, %% \n"); //Burada % koyunca kırmızı renkte oluyordu. Çözümünü araştırınca %d %c bekliyormuş. %% yapınca normal algılıyormuş
        printf("Oyun tahtasini karakter karakter giriniz: (%d satir)\n", N - (N/2));
        for ( i = N/2; i < N; i++) { //PDF'te ben kontrol modunu da N/2 satır diye anladım, eğer öyle değilse kullanıcı "\n" girene kadar string olarak alırdık hocam.
            for ( j = 0; j < M; j++) { 
                // Boşlukları atlamak icin " %c" koydum
                char temp;
                int gecerli = 0; // Uygun karakter olduğu sürece alsın
                do {
                	scanf(" %c", &temp); // Boşlukları atlamak için " %c"
                	if(temp == '*' || temp == '/' || temp == '+' || temp == 'O' || temp == '%') {
                		gecerli = 1;
					} else {
						printf("\n Gecersiz karakter girdiniz! Lutfen karakter listesini baz aliniz!\n");
					}
				} while(gecerli == 0);
					tahta[i][j] = temp;
        ...
        printf("\nTahtaniz kontrol moduna hazir, baslayabilirsiniz\n");
    }
